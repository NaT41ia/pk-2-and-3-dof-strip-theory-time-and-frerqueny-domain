...
Struct = struct('b', 1, 's', 7.5, 'a', -0.2, 'EI', 4*10^7, 'GJ', 8*10^6, 'x_alpha', 0.04, 'Mw', 400, 'Ma', 400, 'k_beta', 1*10^5);
...
y = 0:dy:Struct.s; %Vector to parametrize span of the wing
...
aileron_deflection = ones(1,length(y)); %Assuming the deflection is constant along y and 1 only where it is present
...
[A_struct, inv_A_struct,  C_struct, omega_nat] = matrices_struct(Struct,bending,bending_deflection,torsion,torsion_deflection,aileron, aileron_deflection,y,x, i0_aileronx); %frequency given in rad/s
...
function [A_struct, inv_A_struct, C_struct, omega_nat] = matrices_struct(Struct, bending,bending_deflection,torsion,torsion_deflection, aileron, aileron_deflection,y,x, i0_aileronx)
...
%Returns the structural matrices of the system
    ...
    %Aileron region
    A_struct11a = Ma * trapz(y,trapz(x(i0_aileronx:end),torsion(:, i0_aileronx:end).^2,2));
    A_struct12a = Ma * trapz(y,trapz(x(i0_aileronx:end),torsion(:, i0_aileronx:end).*aileron(:, i0_aileronx:end),2));
    A_struct13a = Ma * trapz(y,trapz(x(i0_aileronx:end),torsion(:, i0_aileronx:end).*bending(:, i0_aileronx:end),2));

    A_struct21a = A_struct12a;
    A_struct22a = Ma * trapz(y,trapz(x(i0_aileronx:end),aileron(:, i0_aileronx:end).^2,2));
    A_struct23a = Ma * trapz(y,trapz(x(i0_aileronx:end),bending(:, i0_aileronx:end).*aileron(:, i0_aileronx:end),2));

    A_struct31a = A_struct13a;
    A_struct32a = A_struct23a;
    A_struct33a = Ma * trapz(y,trapz(x(i0_aileronx:end),bending(:, i0_aileronx:end).^2,2));
     ...
     A11 = A_struct11w + A_struct11a ;
     A12 = A_struct12a;
     A13 = A_struct13w + A_struct13a;
    ...
    aileron_strain = gradient(aileron_deflection,y);
    C_33 = EI*trapz(y,(bending_strain).^2);
    C_22 = k_beta* trapz(y, (aileron_deflection).^2);
